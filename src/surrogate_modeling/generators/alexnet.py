...
def generate_random_config(num_configs, output_csv='alexnet_random_configs.csv'):
    ...
    for _ in range(num_configs):
        ...
        for i in range(1, 6):
            config[f'clb{i}_kernels'] = random.choice([192, 256, 384, 512])
            config[f'clb{i}_kernel_size'] = random.choice([3, 5, 7, 11])
            config[f'clb{i}_stride'] = random.choice([1, 2, 4])
            config[f'clb{i}_activation'] = random.choice(['relu', 'tanh', 'sigmoid'])
            ...
            if i in [1, 2]:  # Only 1 & 2 have LRN
                ...
                config[f'clb{i}_lrn_beta'] = round(random.uniform(0.5, 1), 2)
        ...
        for i in range(1, 4):  # First 3 FC layers (before final classification)
            ...
            if neurons != 0:
                config[f'fc{i}_init'] = random.choice(['xavier', 'kaiming', 'normal'])
        ...
        config['fc4_init'] = random.choice(['xavier', 'kaiming', 'normal'])
        config['fc4_dropout'] = round(random.uniform(0.1, 0.7), 2)
        config['fc4_regularization'] = random.choice(['l2', 'none'])


        # === Training Hyperparameters ===
        config['learning_rate'] = round(random.uniform(0.0001, 0.01), 5)
        config['num_epochs'] = random.choice([10, 20, 30])
